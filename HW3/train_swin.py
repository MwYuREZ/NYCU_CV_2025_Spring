...
from torchvision.models import swin_t, swin_s, swin_b, swin_v2_t
...
def train_model():
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        if map50 > best_map50:
            torch.save(model.state_dict(), Path(OUT_DIR)/'swin_best_map50.pth')
            best_map50 = map50
            best_epoch = epoch
    ...
    plt.grid(True)
    plt.savefig('Validation_mAP50.png')

    print('Training curve and Validation mAP saved')
    
    print(f"Training complete. Best mAP:{best_map50} with epoch:{best_epoch}")
